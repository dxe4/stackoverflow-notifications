...
class Model:
    ...
    def is_exception_overflow(self):
        """
            Check if too many exceptions are generated by the script,
            and use exit_ if something is wrong. Can't raise exception on different thread.
        @return: True if too many exceptions happen
        """
        ...
        if len(self.exception_timestamps) > 10:
            ...
            if len(new_list) < 2:#ensure that the last 3 exceptions dont have the same timestamp
                return True
        return False

    def update_questions(self, questions:list):
        """
            Make a new questions list with the new questions found,
            and save questions all ready show to previous_questions
...
class Scrapper:
    ...
    def login(self, email:str, password:str):
        ...
        if self.driver.current_url != Scrapper.WEBSITE:#if allready logged in need to skip
            self._login(email, password)
        del self.model.password#security reasons
    ...
    def _find_questions(self, html:str):
    ...
    def wait_for_questions(self, ui_signal):
        """
            Main loop, wait for new questions and when found update model/ui
        @param ui_signal:
        """
        while True:
            try:
...
class UIThread(QtCore.QThread):
    ...
    def __init__(self, scrapper:Scrapper, model:Model):
